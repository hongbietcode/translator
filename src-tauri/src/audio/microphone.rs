...
impl MicCapture {
    ...
    /// If `device_name` is Some, uses that specific device; otherwise uses the default input device.
    ...
    pub fn start(
        ...
        let input_devices: Vec<(String, Device)> = host
            .input_devices()
            .map(|devs| devs.filter_map(|d| d.name().ok().map(|n| (n, d))).collect())
            .unwrap_or_default();
        let device_names: Vec<&str> = input_devices.iter().map(|(n, _)| n.as_str()).collect();
        println!("[Mic] Available input devices: {:?}", device_names);
        ...
        let device: Device = if let Some(ref name) = device_name {
            input_devices
                .into_iter()
                .find(|(n, _)| n == name)
                .map(|(_, d)| d)
